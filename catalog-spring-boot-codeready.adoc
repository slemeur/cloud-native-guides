...
which is already configured in CodeReady Workspaces: `*right click on 'catalog-spring-boot'*` project in the project explorer then, `*click on 'Commands > Build > build'*`

You should get a **BUILD SUCCESS** message in the build logs, otherwise the build has failed.
